...
### v43 — Admin & Sistem: Alert Login, Backup Otomatis, Branding, 2FA, Moderator (Task 60-c)
- **Konteks**: batch ke-3 dari 24 fitur disetujui (#21–#25). Dikerjakan subagent (commit per-fitur: v43.1–v43.4) + main agent menuntaskan klien 2FA, E2E, dan penutupan (subagent terputus konteks kedua kali; strategi commit kecil per fitur terbukti menyelamatkan 4/5 pekerjaan).
- **(1) Notifikasi login perangkat baru**: saat perangkat BARU terikat ke akun (INSERT devices di user:auth) → event `admin:new_login` {userId, userName, deviceId(pendek), ip, userAgent, at} ke room admins → notice "🔓 Login baru: …". Terverifikasi: login fixture dgn deviceId baru → admin menerima event lengkap.
- **(2) Backup otomatis terjadwal**: `runAutoBackup()` memanggil `scripts/make-backup.sh` (hardened v36; fire-and-forget) pada jam WIB yang BISA DIATUR (setting `autoBackupAt`, default "03:07"; anti-dobel harian via setting `lastAutoBackup`) → audit + `admin:auto_backup` {ok, at} ke admins. Event: `admin:auto_backup_get` (status jadwal+hasil terakhir), `admin:auto_backup_now` (jalankan sekarang). UI: kartu "Backup otomatis terjadwal" di tab Pusat (input HH:MM + simpan + jalankan sekarang + status terakhir).
- **(3) Branding kustom**: setting `appLogo` (URL /api/media, validasi pola) + `accentColor` (hex #rrggbb) masuk AppSettings + admin:settings:set + siaran app:settings:update. UI Pengaturan: unggah logo (upload → /api/upload) + preview + input color "Warna aksen" + "Reset warna". Terapkan global: `applyAccentColor()` (chat-utils) men-set CSS vars --primary/--ring/--sidebar-primary/--sidebar-ring + kontras teks berbasis luminansi; dipanggil AdminPanel & Messenger saat settings berubah; **perbaikan main agent: AdminPanel kini FETCH `admin:settings:get` langsung setelah login** (dulu hanya mendengar broadcast — aksen tidak muncul setelah reload). Logo tampil di header admin/user/login.
- **(4) 2FA TOTP admin** (RFC 6238 murni node:crypto — base32 sendiri, 30 dtk, 6 digit, toleransi ±1 langkah; default MATI): settings `totpSecret`(aktif)/`totpSecretPending`/`totpEnabled`; event `admin:totp_state`/`totp_setup`(secret+otpauth URI)/`totp_enable`(verifikasi pending)/`totp_disable`(verifikasi aktif) — semua adminGuard+destructive+audit. Gate di `admin:auth`: enabled tanpa kode → `{ok:false,error:'TOTP_REQUIRED'}`, salah → `TOTP_INVALID` (dihitung ke anti-bruteforce). Klien: login — input 6 digit muncul saat gate, auto-submit saat lengkap, error jelas; Pusat — seksi "Keamanan — verifikasi 2 langkah" (status + aktifkan [secret+otpauth+salin+konfirmasi] + matikan). E2E UI penuh: aktifkan via dialog (kode dihitung dari secret yang tampil), logout, login → gate muncul → kode benar → masuk; matikan lagi via UI (keadaan bersih).
- **(5) Multi-admin + role 'moderator'**: role baru = lihat semua, tanpa aksi merusak. `admin:auth` menerima password AKUN moderator (fallback bcrypt ke users.role='moderator') → sesi room admins dengan `actorRole:'moderator'`. Guard baru `adminGuardDestructive` menolak moderator pada ±60 event destruktif (FORBIDDEN); event baca tetap jalan. `admin:user_role` {userId, role user↔moderator} (admin penuh saja; tak boleh menyentuh admin). Klien: badge "Moderator" di header + tombol destruktif disembunyikan + toast "Mode moderator — akses terbatas". E2E: login moderator ✓, reset FORBIDDEN ✓, leaderboard ✓, promosi/demosi ✓.
