/** v48 — fixture akun uji: UjiV48 (pw uji48) + percakapan dengan admin. */
...
if (!u) {
  ...
  const hash = Bun.password.hashSync("uji48", { algorithm: "bcrypt", cost: 10 });
  db.run(
...
if (cnt === 0) {
  db.run("INSERT INTO messages (conversation_id, sender_id, content, created_at, type) VALUES (?, 'admin', ?, ?, 'text')", [conv.id, "Selamat datang di v48! Coba stiker & panel media.", now - 60000]);
