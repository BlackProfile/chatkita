/**
 * Util Task 62 — fixture E2E v46 (Konsolidasi).
 * create : UjiBrowser62 (pw uji62, 3 pesan, 1 memuat "rahasia")
 *          + UjiBrowser62B (pw uji62, khusus uji hapus akun)
 * cleanup: hapus total kedua user + pesan + reaksi + reads + devices
 * Pemakaian: bun .zscripts/t62-fixture.ts create | cleanup
 */
...
if (process.argv[2] === "create") {
  ...
  const now = Date.now();
  const idA = ensureUser(db, "UjiBrowser62", "uji62");
  const idB = ensureUser(db, "UjiBrowser62B", "uji62");
  ...
  if (count === 0) {
    const texts = ["Halo admin, ini pesan uji t62 nomor satu", "rahasia62: kode uji konsolidasi", "Oke sip, terima kasih admin"];
  ...
  out(`create OK — UjiBrowser62=${idA} convA=${convA} pesan=${total}; UjiBrowser62B=${idB}`);
  db.close();
} else if (process.argv[2] === "cleanup") {
  const db = new Database(DB_PATH);
  for (const name of ["UjiBrowser62", "UjiBrowser62B"]) {
  ...
  const sisa = (db.query("SELECT COUNT(*) AS v FROM users WHERE name LIKE 'UjiBrowser62%'").get() as { v: number }).v;
  out(`sisa fixture t62 = ${sisa}`);
  db.close();
} else {
  out("pemakaian: bun .zscripts/t62-fixture.ts create | cleanup");
}
